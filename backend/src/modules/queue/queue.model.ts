...
export interface IQueue extends Document {
  name: string;
  location: string; // NEW: Added for Issue #131
  isActive: boolean;
  nextSequence: number;
...
const queueSchema = new Schema<IQueue>(
  {
    ...
    // NEW: Added location to distinguish queues (e.g., multiple "Registrar" counters)
    location: {
      type: String,
      required: true, 
    },
...
// Updated Index: Prevent duplicate queues (Same name + Same location)
